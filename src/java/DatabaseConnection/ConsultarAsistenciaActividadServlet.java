...
import DatabaseConnection.ModeloAsistenciaActividad;

/**
 *
 * @author Pc
 */
@WebServlet(name = "ConsultarAsistenciaActividadServlet", urlPatterns = {"/ConsultarAsistenciaActividadServlet"})
public class ConsultarAsistenciaActividadServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("consultarAsistencias".equalsIgnoreCase(accion)) {
            ...
            if (id_miembro == null || id_miembro.isEmpty()) {
                ...
                try {
                    con.ConexionBdMySQL();
                    String query = "SELECT * FROM asistencia_actividad WHERE id_miembro = ?";
                    List<ModeloAsistenciaActividad> asistencias = new ArrayList<>();
                    ...
                    try (PreparedStatement statement = con.getConexionBd().prepareStatement(query)) {
                        ...
                        while (rs.next()) {
                            ModeloAsistenciaActividad asistencia = new ModeloAsistenciaActividad();
                            asistencia.setIdAsistencia(rs.getInt("id_asistencia"));
                            asistencia.setIdMiembro(rs.getInt("id_miembro"));
            ...
            response.sendRedirect(request.getContextPath() + "/cliente/consultarAsistenciaActividad.jsp");
        }
